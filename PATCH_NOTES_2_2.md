...
- **Twenty-nine legacy alias power types now have real editor forms.** Legacy types carried by the compatibility layer (the four persistent-effect aliases, seven `condition_passive` aliases, fourteen `action_on_event` aliases, and four stragglers) had no schema branch of their own, so the editors fell back to a raw-JSON box and left the author to spell every field from memory. All twenty-nine are now modelled, and the allowlist that exempted them is empty. Alongside that: the authorable power type list is now the union of the native registry and what the two compatibility dispatch switches actually dispatch, which takes it from a hundred and fifty-two ids to five hundred and fifty-five and drops the legacy pack corpus from one thousand and eighty-eight files rejected on `type` alone to six; the five verb schemas gained the `origins:`, `apoli:`, and `apugli:` spellings they never carried, taking a per-verb probe over that same corpus from zero of five thousand four hundred and twenty valid to four thousand seven hundred and fourteen; scalar-or-array reference fields (twenty-two of them) render as proper pickers in the in-game editor instead of raw JSON; and the top-level `enabled` kill switch and `action_on_event`'s `hit_dealt` event are declared where they were being read but not described. The native types that fall back to codec reflection (`multiple`, `status_effect`, `glow`, and `night_vision` among them) get their display name, description, hidden flag, and `required_mods` rows back; those keys belong to no power's config record, so the fallback path had never offered them even though a datapack could always write them by hand.
- **The schemas no longer reject JSON the mod itself accepts.** Four separate cases where the editors flagged a perfectly good file as invalid. The layer schema never declared `standalone`, took `name` as a plain string only, and allowed only bare-id or single-object origin entries, while the layer loader accepts all three of the shapes it left out. The item-condition schema demanded a `type` that the parser has never needed, so the typeless `id` / `item` / `tag` shorthand, which NeoOrigins' own Jianxian powers use, failed the mod's own schema. `prevent_action` declared its two enums in lowercase only, though the power upper-cases before matching, so the sixteen shipped powers written in `SCREAMING_CASE` were all reported invalid while working perfectly in game. And an active power's `key` was specced as an integer from 1 to 64, while the loader has always read it three ways: a number is a named-hotkey pool slot, a bare string is used as a keybind translation key, and an object supplies a key plus a `continuous` flag, so a pack binding a power to `"key.jump"` ran fine, failed validation, and could not be expressed in the form at all. All four now describe what the loader actually does.
...
- **Lava vision changes what you can see through lava again.** The fog handler was stubbed out during the 26.1 render rework, on the reading that `ViewportEvent.RenderFog` had stopped being cancellable and so could no longer replace vanilla's planes. It had, but that turns out not to matter: the event now carries the live fog data and its setters write straight through to it, so there is nothing left to cancel. The handler is restored, and `lava_vision` pushes the fog planes back here the same way it does on 1.21.1, which until now was the only build where the power did anything visible. One extra step this version needs: vanilla pins the sky and cloud distances to the lava fog's end as it sets them, and those are not what the two plane setters touch, so left alone they draw as a hard band at arm's length in front of a view that is otherwise clear. They now follow the far plane out with everything else.
...
- **Two Blacksmith quality-craft bugs.** A full set of quality-crafted armour granted a single toughness bonus instead of one per piece, because all four pieces shared one attribute-modifier id and the game deduplicates modifiers by id; each slot now carries its own id, so head, chest, legs, and feet stack independently. Mining speed and attack damage were never affected, since only one held item is ever in play. Separately, taking a crafted item with shift-click distributes it into your inventory before the crafted-item event fires, so the quality bonus was applied to a stack that had already been merged away and vanished. The buff is now applied to the result slot as it is assembled, which covers both the click-take and shift-click paths; applying it twice is harmless, so nothing double-dips.
...
- **Three things the editors offered on this branch that the build did not actually honour.** `neoorigins:variable` was declared in the power table, so the schema, the pack creators, and the type picker all listed it, but it was never registered, and an unregistered type is not a soft failure: the power file is dropped whole at load with no error, so every pack that took the editor at its word silently lost its counters and any resource condition gating on them. The two slime powers spelled three of their fields `split_max_h_p`, `levels_per_hp`, and `max_bonus_h_p` in everything author-facing, because the key is derived from the record component name and `HP` camel-cases to `h_p`; the codecs have only ever parsed `split_max_hp`, `levels_per_hp`, and `max_bonus_hp`, so editing any of the three in the creator wrote a key nothing read and the value fell back to its default. And the `body_temperature` condition and `modify_temperature` action shipped with the Cold Sweat integration were registered as verbs but never added to the parsers' known-type sets. Datapacks using them always worked, since the runtime dispatches through the compatibility registry, but the in-game creator builds its pickers from those sets, so neither was offered, and a draft that referenced one was reported as an unknown type and refused by the save gate.
- **The water-damage config now actually reaches the water powers.** `power_overrides.toml` declares a `multiplier` for the water damage and water weakness powers (and `damage_per_second` for Enderian), but eight of the ten were still written as raw `condition_passive` powers with the damage baked into the action's `amount`, a shape that has no such field, so the override was injected and then ignored. Turning water damage down to `0` still dealt full damage every interval. All eight are now `neoorigins:damage_in_water`, whose alias expander reads `damage_per_second`, scales it by `multiplier`, and substitutes a no-op when the result is not positive, so `0` truly disables the damage instead of calling for a zero-strength hit. The damage values themselves are unchanged; the damage source becomes drown rather than magic, matching the two powers that already went through the alias.
- **The hotkey and skill keybinds are named in the Controls screen.** Skill 5, Skill 6, and the pool of up to sixty-four named hotkeys were registered but had no translation keys, so the Controls screen listed all sixty-seven rows as raw ids (`key.neoorigins.hotkey.1` and friends) under an unnamed category. That is also the list a player has to read to bind a pack-declared `"key": N` power, so the one screen that makes those powers usable was the one screen that would not say what they were.
...
- **Four authorable action fields were being dropped at parse time on this branch.** Each was documented, offered by the editors, or both, and each was thrown away before it reached the game. `action_on_event` never compiled its `item_condition`, so a power meant to fire only on a named item fired on every item use instead. The fix also maps `action_on_item_use`'s `trigger` onto the start and finish of the use, which is what that field is for. `equipped_item_action` read its nested action only from the undocumented key `action`, while [ACTIONS.md](docs/ACTIONS.md) and Apoli both call it `item_action`, so anyone following the documentation got a power that did nothing and said nothing; `item_action` is now read first and `action` stays accepted for packs already built against it. `damage` understood only the seven `source.name` keywords, which put every other damage type, modded ones especially, out of reach; an explicit `damage_type` id is now resolved against the damage-type registry at dispatch and takes priority, with an unknown id warning once and falling back rather than failing the action. And `modify_inventory`'s `slot` was documented but never read, which is the destructive one: a slot-scoped `consume` silently widened to the whole inventory and destroyed every match. The name is now validated at parse time and resolved to a container index per execution, since mainhand depends on which hotbar slot is selected.
...
- **The JEI, Jade, and REI integrations ship again on Minecraft 26.2.** All three plugins were commented out of this branch behind a note saying none of those mods had a 26.2 build. They do now (JEI on its 30.x line, Jade 26.2.8, and REI 26.2.821), so the plugins are compiled and published rather than carried as dead source. JEI gets an information page on the Orb of Origin explaining what it does; REI adds the same orb to its entry list; and Jade shows a looked-at mob's NeoOrigins origin in its tooltip overlay. All three stay compile-only soft dependencies, so a game without them is unaffected. JEI is deliberately pinned to `30.11.0.66` rather than the newest build: from `30.11.0.67` onward it requires a NeoForge newer than this branch's floor, and moving the floor to chase it is not a trade worth making for an item tooltip. FTB Ultimine is the one that stays off: its maven stops at `26.1.2.5`, so the restriction-handler bridge that turns `neoorigins:ultimine` into real vein-mining cannot be compiled here and the power is an inert marker. The creator listed it anyway, and choosing it produced a power that loaded, appeared on the origin, and did nothing with no indication why; the type picker now hides it the same way it hides retired types, while the type itself stays registered so an origin ported from the 26.1 branch still loads, and the loader logs a warning naming the missing integration when a pack defines one.

### Documentation

- **Four reference gaps this branch was carrying are closed.** [THEMING.md](docs/THEMING.md) did not exist here at all, even though every client theme class and the whole parchment asset set ship on this branch, so the THEMING.md links in [API.md](docs/API.md) and [CLIENT_CONFIG.md](docs/CLIENT_CONFIG.md) pointed at nothing; the page is now written against this branch's theme code, including the `flat` and `panel_color` fields that are the only route to a flat theme, the `Identifier` type name this version uses, and the fact that the `classic_picker_style` accessibility option short-circuits theme selection entirely, which is the one thing that can silently override a pack's theme. `neoorigins:elytra_flight` was the only registered power type on any branch with no entry in [POWER_TYPES.md](docs/POWER_TYPES.md). `neoorigins:cast_iron_spell` was registered here but documented only on the other two branches. And the `damage` action's `damage_type` field has been read since 2.0.x and described by the editors, while [ACTIONS.md](docs/ACTIONS.md) still listed `source.name` as the only way to choose a damage source; unlike that fixed keyword list, `damage_type` resolves against the damage-type registry, so it reaches datapack-defined types.
- **The KubeJS integration is marked 1.21.1 only, and the other disabled integrations now name their real blocker.** [KUBEJS.md](docs/KUBEJS.md) read as though KubeJS support were universal with the 26.x builds merely lagging. It is not: the NeoOrigins globals, the `kubejs_callback` action, and the `js_custom` and `js_active` power types are absent from both 26.x builds, and that is worth spelling out because an unregistered power type is not a soft failure. A `js_custom` power copied into a 26.x pack is dropped whole at load, with no error and no partial behaviour, so it looks like the file simply vanished. The page now says so above the table of contents rather than several sections in. The same pass corrected the stated reasons for the integrations that stay off: EMI and The One Probe have no build for any 26.x at all, not merely none for 26.1 as the old note claimed; FTB Ultimine's maven tops out at 26.1.2.5; and [COMPATIBILITY.md](docs/COMPATIBILITY.md) no longer describes FTB Ultimine as working on this branch. Nothing was re-enabled by that pass: only the reasons changed, and each now carries the date it was checked.
