...
| `allow_water_surface` | boolean | Default `false`. If no dry land column is found, accept the topmost water column. The player spawns feet-in-water, head above. |
| `allow_ocean_floor` | boolean | Default `false`. If no dry land column is found, accept the seabed. The player spawns submerged on the floor (needs water breathing to survive). |
