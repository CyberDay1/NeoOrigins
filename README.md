...
- **Night-vision toggle**: press K to switch off origin-granted night vision when you'd rather have the dark back; it starts on, toggles only your own view, and persists across relogs. Rebindable in *Controls → Key Binds → NeoOrigins*, alongside Skill 1-6 (V/G/N/B), Class Skill (H) and View Origin Info (O). See [docs/CLIENT_CONFIG.md#built-in-keybinds](docs/CLIENT_CONFIG.md#built-in-keybinds)
...
- **JEI / EMI / REI integration**: JEI and EMI show an info panel for the Orb of Origin; REI lists the Orb of Origin as an entry
...
- `neoorigins/admin.toml`: command blacklist, command access, dimension restrictions, unique origin layers, entity exclusions, compat filtering, debug flags
