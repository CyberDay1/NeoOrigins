...
```
After (dormant alias; swaps with NEAREST entity in radius; legacy used a look-direction raycast):
```json
{
  "type": "neoorigins:active_ability",
...
```
After (dormant alias; lossy, legacy fired 3–4 fireballs with spread; alias fires one):
```json
{
  "type": "neoorigins:active_ability",
  "entity_action": {
