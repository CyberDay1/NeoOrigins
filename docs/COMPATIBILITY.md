...
| Pack with no `pack.mcmeta` (or shipped as a `.jar`) | Vanilla's folder repository source rejects `.jar` files outright and refuses folders missing `pack.mcmeta`, and Origins mod JARs ship without one, so the pack never even registers. | `originpacks/` entries are scanned by NeoOrigins' own pack finder: `.jar`, `.zip` and plain folders are all accepted, and when `pack.mcmeta` is absent compatible metadata is synthesized so the pack registers anyway. A real `pack.mcmeta`, when present, is read normally and wins. |
...
|---|---|---|
| **Curios API** | `curios` | Lets conditions inspect equipped curio slots: the `equipped_item` condition's `accessory` slot (optionally narrowed by `slot_type`) and umbrella detection both read worn curios, and `neoorigins:keep_inventory` keeps matching curio-slot items through death. Reflection-based, no hard dependency. Curios is the only accessory mod read on this build: there is no Accessories (Wisp Forest) support. |
...
| Mod | Mod id | What it adds |
|---|---|---|
| **KubeJS** | `kubejs` | **Not available on this build.** KubeJS publishes no 26.2 artifact, so the integration is not compiled in: no `NeoOrigins` / `NeoOriginsEvents` globals, no `kubejs_callback` action, no `js_custom` / `js_active` power types. See [KUBEJS.md](KUBEJS.md#availability). |
