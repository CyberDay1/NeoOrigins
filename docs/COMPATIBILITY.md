...
| **FTB Quests** | `ftbquests` | **Tag marker only on this build.** A quest tagged `neoorigins_loot_pool_grant:<table_id>` rolls that loot table once, on completion, into the inventory of every online member of the completing team (overflow drops at their feet). It grants at most once per player per quest-and-table, and a roll that yields no items is not counted. There is **no** NeoOrigins reward type in the quest editor on 26.1: the 1.21.1 build's "Grant Loot Pool" reward has not been ported. Compile-only soft dependency. See [FTB Quests](#ftb-quests). |
