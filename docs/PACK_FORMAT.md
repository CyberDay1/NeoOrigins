...
Minecraft 1.21.1 uses data-pack format `48` and resource-pack format `34` (the
`pack_version` in the 1.21.1 game jar's `version.json`).
...
- `pack_format` is required. `supported_formats` is optional: a range written as
  `[min, max]` or `{"min_inclusive": min, "max_inclusive": max}`. The game ignores
  the range unless it contains `pack_format`.
- A pack that carries both `data/` and `assets/` and is also installed as a resource
  pack can declare `"pack_format": 48, "supported_formats": [34, 48]` so it is listed
  as compatible in both roles.
- `min_format` / `max_format` are Minecraft 26.x fields. 1.21.1 ignores them.
- In `config/originpacks/` the file is optional: a pack without one, or with one the
  game cannot read, is loaded with generated metadata that marks it compatible. A
  pack in `<world>/datapacks/` needs a readable `pack.mcmeta`, or the game skips it.
