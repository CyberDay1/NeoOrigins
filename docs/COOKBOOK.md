...
- The `key` entry shape changed between Minecraft versions. On 26.1 and
  later each key is a bare ingredient string:
  `"G": "minecraft:gold_ingot"`, as above. On 1.21.1 each key maps to an
  object instead: `"G": { "item": "minecraft:gold_ingot" }`. Compare
...
| `neoorigins:aquatic_buoyancy` | -0.08 `minecraft:gravity` while submerged, cancelling in-water sink (Neutral Buoyancy) |
