...
| `block_condition` | BlockCondition | no | — | Block-position gate for block events (`block_break`, `block_place`, `block_use`). Supports `block`/`id`, `in_tag`, `and`/`or`, and the positional `offset` wrapper (see below). Ignored on other events. |
