 ...
 * different ways — a direct static call here and on {@code 26.2}, a render-state
 * {@code ContextKey} on {@code 26.1} — so the expectation below describes this
 * branch only, exactly as the sneak-ledge tests each carry their own epsilon.
 */
// hub: neoorigins/elytra-draw-gate.md
class ElytraLayerWiringTest {

    /** The layer's draw entry point on the 1.21.1 line. */
    private static final String ENTRY_POINT = "shouldRender";
    ...
    void theDrawEntryPointDelegatesToTheGate() {
        List<String> calls = callsIn(ENTRY_POINT);
    ...
    void theDrawEntryPointStillAsksWhetherTheWingsAreWantedAtAll() {
        ...
        assertTrue(callsIn(ENTRY_POINT).contains("shouldRenderElytra"),
            ENTRY_POINT + " must still read the plain render_elytra flag");
    }

    private static List<String> callsIn(String methodName) {
