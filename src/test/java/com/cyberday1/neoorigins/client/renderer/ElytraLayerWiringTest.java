 ...
 * different ways - a render-state {@code ContextKey} here on {@code 26.1}, a
 * direct static call on {@code 1.21.1} and {@code 26.2} - so the expectation below
 * describes this branch only, exactly as the sneak-ledge tests each carry their own
 * epsilon. That is also why this had to be written three times rather than
 * cherry-picked.
 */
// hub: neoorigins/elytra-draw-gate.md
class ElytraLayerWiringTest {

    /** The layer's draw entry point on the 26.1 line. */
    private static final String ENTRY_POINT = "submit";
    private static final String GATE = "shouldDrawWings";
    /** 26.1 carries the always state as a render-state ContextKey, not a static call. */
    private static final String ALWAYS_LOOKUP = "RENDER_ELYTRA_ALWAYS_KEY";
    private static final String RENDER_LOOKUP = "RENDER_ELYTRA_KEY";
