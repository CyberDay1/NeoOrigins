...
public final class PowerSuppression {
     ...
     * than spamming.
     *
     * <p>Both halves of the feedback are spelled out longhand here because this
     * branch dropped the two shorthands. {@code displayClientMessage} is now
     * {@code sendSystemMessage} with the overlay flag, and {@code playNotifySound}
     * is gone entirely: a sound only the one player hears has to go out as a
     * direct packet, the way vanilla's raid horn does it.
     *
    ...
    public static boolean refuseActivation(ServerPlayer player, boolean announce) {
        ...
        if (announce) {
            ...
            player.connection.send(new ClientboundSoundPacket(
                Holder.direct(SoundEvents.FIRE_EXTINGUISH), SoundSource.PLAYERS,
                player.getX(), player.getY(), player.getZ(), 0.5F, 1.6F, 0L));
        }
        return true;
    }
}
