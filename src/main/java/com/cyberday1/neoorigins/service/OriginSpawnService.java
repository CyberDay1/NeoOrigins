...
public final class OriginSpawnService {
    ...
    public static void applyRespawnSpawnOverrides(ServerPlayer player) {
        ...
        // the chain now spans ticks.
        final boolean hasRespawnPosition = player.getRespawnPosition() != null;
    ...
    private static boolean stillHasOrigin(ServerPlayer player, ResourceLocation originId) {
        PlayerOriginData data = player.getData(OriginAttachments.originData());
    ...
    private static void apply(ServerPlayer player, LocationCondition.SpawnTarget target) {
        Vec3 pos = target.pos();
        if (target.level() == player.serverLevel()) {
