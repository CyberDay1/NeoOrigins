...
public final class BiomeSpawnSearch {
    ...
    public static Optional<Query> capture(ServerLevel level, BlockPos searchOrigin, Predicate<Holder<Biome>> predicate) {
        ...
        // Mirrors vanilla's band. On 26.x the accessors are getMinY()/getMaxY()
        // and getMaxY() is INCLUSIVE (minY + height - 1), where 1.21.1's
        // getMaxBuildHeight() was exclusive (minY + height). Vanilla 26.2's
        // BiomeSource#findClosestBiome3d asks for
        // Mth.outFromOrigin(y, level.getMinY() + 1, level.getMaxY() + 1, step),
        // so the "+ 1" on the top is required to keep the same band the 1.21.1
        // branch searches — dropping it would silently shave the top sample
        // level off every search.
        ...
        // and searches from BlockPos.ZERO — and the Nether's band starts at
        // Y = 1, so an unclamped Y = 0 origin would make every Nether biome
        // search a zero-sample miss (blazeling, cave/forest dragon). Clamp here
        // so the spiral sees exactly what vanilla would have seen.
