 ...
 * ({@link net.minecraft.server.MinecraftServer#getTickCount()}) as its clock
...
public final class FirstPickGraceTracker {
    ...
    public static void grant(ServerPlayer sp, int ticks) {
        EXPIRY_TICK.put(sp.getUUID(), sp.getServer().getTickCount() + ticks);
    }
    ...
    public static boolean isActive(ServerPlayer sp) {
        ...
        if (sp.getServer().getTickCount() < expiry) return true;
