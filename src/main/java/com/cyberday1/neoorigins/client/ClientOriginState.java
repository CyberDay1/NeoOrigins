...
public class ClientOriginState {
    ...
    public static void openSelectionScreen(boolean isOrb, boolean forceReselect,
        ...
        // No default arm, so a new enum value fails the build. This switch must stay
        // in this client-package class: RuntimeDistCleaner walks NEW opcodes in
        // common-side classes, so a Screen reference from NeoOriginsNetwork crashes a
        // dedicated server on boot.
        mc.gui.setScreen(switch (NeoOriginsClientConfig.pickerLayout()) {
            ...
            case GRID -> new com.cyberday1.neoorigins.screen.OriginGridSelectionScreen(isOrb, forceReselect, scopedLayers);
