...
public class ResourceHudOverlay {
    ...
    public static void onRenderGui(RenderGuiEvent.Post event) {
        ...
        for (var entry : resources.entrySet()) {
            ...
            g.drawString(mc.font, label, baseX + (BAR_WIDTH - labelW) / 2, y - 9, LABEL_COLOR, false);

            // Value readout below bar
            String readout = res.value() + "/" + res.max();
            int readoutW = mc.font.width(readout);
            g.drawString(mc.font, readout, baseX + (BAR_WIDTH - readoutW) / 2, y + BAR_HEIGHT + 2, LABEL_COLOR, false);
