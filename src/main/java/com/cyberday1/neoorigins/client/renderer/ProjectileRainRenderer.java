...
/**
 * 26.2 renderer for {@link ProjectileRainVfxEntity}. Draws the whole ring of
 ...
 * {@code landingTick(i)} schedule (snapshotted into the render state), so the
 * visual thunk lands on the exact tick the server applies that blade's damage.
 * See the 1.21.1 twin for the {@code EntityRenderer.render()} variant.
 *
 ...
 * {@code +90°} rotation about X.
 */
public class ProjectileRainRenderer
        extends EntityRenderer<ProjectileRainVfxEntity, ProjectileRainRenderState> {
    ...
    @Override
    public ProjectileRainRenderState createRenderState() {
        return new ProjectileRainRenderState();
    }

    @Override
    public void extractRenderState(ProjectileRainVfxEntity entity, ProjectileRainRenderState state, float partialTick) {
        super.extractRenderState(entity, state, partialTick);
        com.cyberday1.neoorigins.api.content.vfx.AbstractVfxRenderState.extract(entity, state);
        state.renderBlades = entity.shouldRenderBlades();
        state.count = entity.getSwordCount();
        state.maxLifetime = entity.getMaxLifetime();
        state.model = entity.getModel();
        state.entityY = entity.getY();
        int count = state.count;
        state.landingTicks = new float[count];
        state.localOffsets = new Vec3[count];
        state.groundYs = new double[count];
        for (int i = 0; i < count; i++) {
            state.landingTicks[i] = entity.landingTick(i);
            state.localOffsets[i] = entity.bladeLocalOffset(i);
            state.groundYs[i] = entity.bladeGroundY(i);
        }
    }

    @Override
    public void submit(ProjectileRainRenderState state, PoseStack poseStack,
        ...
        int count = state.count;
        float maxLife = state.maxLifetime;
        BakedMeshModel model = modelFor(state.model);

        for (int i = 0; i < count; i++) {
            float landing = state.landingTicks[i];
            ...
            double groundDelta = state.groundYs[i] - state.entityY;
            ...
            collector.submitCustomGeometry(poseStack, RENDER_TYPE, (pose, consumer) ->
                model.renderTinted(pose, consumer, TINT_R, TINT_G, TINT_B, alphaFinal,
