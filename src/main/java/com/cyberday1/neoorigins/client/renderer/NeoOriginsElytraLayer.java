 ...
 * looks the inputs up by {@code state.id} and hands them over. The texture comes from
 * {@link ClientElytraFlightState#textureFor(int)}, which falls back to the vanilla
 * elytra texture when no custom one was set.
 *
...
public class NeoOriginsElytraLayer<S extends AvatarRenderState, M extends EntityModel<? super S>>
    ...
    public void submit(PoseStack poseStack, SubmitNodeCollector submitNodeCollector, int lightCoords,
        ...
        if (!shouldDrawWings(state.isFallFlying,
                             ClientElytraFlightState.shouldRenderElytra(state.id),
                             ClientElytraFlightState.alwaysRendersElytra(state.id),
