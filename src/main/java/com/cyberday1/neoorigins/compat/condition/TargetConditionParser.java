...
public final class TargetConditionParser {
     ...
     *
     * <p>26.x note: entity serialization flows through {@code ValueOutput}
     * rather than raw {@code CompoundTag}; we bridge via {@code TagValueOutput},
     * and {@code TagParser.parseTag} is gone — use {@code parseCompoundFully}.
     */
    private static TargetCondition parseNbt(JsonObject json) {
        ...
        return (t, a) -> {
            var out = net.minecraft.world.level.storage.TagValueOutput.createWithContext(
                net.minecraft.util.ProblemReporter.DISCARDING, t.level().registryAccess());
            t.saveWithoutId(out);
            return net.minecraft.nbt.NbtUtils.compareNbt(expected, out.buildResult(), true);
        };
    }
    ...
    private static TargetCondition parseInEntityTypeTag(JsonObject json) {
        ...
        TagKey<EntityType<?>> tag = TagKey.create(Registries.ENTITY_TYPE, Identifier.parse(id));
        return (t, a) -> t.getType().getTags().anyMatch(tk -> tk.equals(tag));
    }
