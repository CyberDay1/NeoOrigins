...
public final class BuiltinActions {
    ...
    private static final ThreadLocal<java.util.Set<Identifier>> ACTIVATE_IN_FLIGHT =
    ...
    static {
        ...
        define("damage",
            (json, ctx) -> {
                ...
                final net.minecraft.resources.Identifier dtId =
                    dtStr.isEmpty() ? null : net.minecraft.resources.Identifier.tryParse(dtStr);
                String sourceType = "";
        ...
        // set_block — place a block. Lift-and-shift of parseSetBlock (26.1:
        // Identifier + lambda-internal BuiltInRegistries.BLOCK.get().value()).
        // Block id is the hard requirement (parser no-ops without it). `keep`
        // optional (parser default false). Legacy code-registered Origins block
        // ids are remapped to their NeoOrigins equivalents.
        //
        ...
        define("activate_power",
            (json, ctx) -> {
                ...
                return player -> {
                    java.util.Set<Identifier> inFlight = ACTIVATE_IN_FLIGHT.get();
                    if (!inFlight.add(pid)) {
        ...
        define("open_layer_picker",
            (json, ctx) -> {
                java.util.List<Identifier> layers = new java.util.ArrayList<>();
                ...
                java.util.List<Identifier> finalLayers = java.util.List.copyOf(layers);
                return player -> {
