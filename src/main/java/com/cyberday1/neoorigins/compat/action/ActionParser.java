...
public final class ActionParser {
    ...
    public static final java.util.Set<String> KNOWN_TYPES = java.util.Set.of(
        ...
        "neoorigins:ignite_attacker", "neoorigins:invert", "neoorigins:kubejs_callback",
        "neoorigins:launch",
    ...
    static EntityAction parseSpawnProjectileRain(JsonObject json, String contextId) {
        ...
        if (projectileId != null && !projectileId.isBlank()) {
            var rl = net.minecraft.resources.Identifier.tryParse(projectileId);
            if (rl != null) {
        ...
        return player -> {
            if (!(player.level() instanceof ServerLevel sl)) return;
            var entity = com.cyberday1.neoorigins.content.ModEntities.PROJECTILE_RAIN.get()
                .create(sl, EntitySpawnReason.MOB_SUMMONED);
            if (entity == null) return;
            ...
            if (projectileType != null) {
                ...
                entity.setLaunchCallback((lvl, groundPos, caster) -> {
                    var proj = projectileType.create(lvl, EntitySpawnReason.MOB_SUMMONED);
                    if (proj == null) return;
    ...
    static EntityAction parseSpawnTelegraph(JsonObject json, String contextId) {
        ...
        return player -> {
            if (!(player.level() instanceof ServerLevel sl)) return;
            var entity = com.cyberday1.neoorigins.content.ModEntities.TELEGRAPH.get()
                .create(sl, EntitySpawnReason.MOB_SUMMONED);
            if (entity == null) return;
