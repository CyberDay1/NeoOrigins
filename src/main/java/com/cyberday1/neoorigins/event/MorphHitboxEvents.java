...
public final class MorphHitboxEvents {
    ...
    /**
     * Entity ids of the players whose frustum culling we turned off, so that we
     * only ever put back what we took — a mod that switched it off for its own
     * reasons keeps it off when the morph ends.
     */
     ...
     * it.
     *
     * <p>Client-side only: the flag exists on every entity but nothing on a
     * server ever reads it, and flipping it there would be noise in the save.
     * The exemption is not free — an exempt player is drawn whenever they are
     * in render distance, whether or not they are behind the camera — which is
     * why it is confined to the handful of morphs whose box genuinely can't
     * stand in for their silhouette.
     */
    private static void updateCulling(Player player, boolean exempt) {
        ...
        if (exempt) {
            if (!player.noCulling) {
                ...
                UNCULLED.add(entityId);
            }
        } else if (UNCULLED.remove(entityId)) {
