...
public class EntityGroupDataManager
     ...
     * @param burnsInSunlight      true if the player catches fire in open daylight (skeleton behaviour)
     */
    ...
    private Map<Identifier, GroupDef> loaded = Map.of();
    ...
    protected Map<Identifier, JsonElement> prepare(ResourceManager resourceManager, ProfilerFiller profiler) {
        ...
        for (var entry : FILE_CONVERTER.listMatchingResources(resourceManager).entrySet()) {
            ...
            Identifier groupId = FILE_CONVERTER.fileToId(fileId);
    ...
    protected void apply(Map<Identifier, JsonElement> object, ResourceManager resourceManager, ProfilerFiller profiler) {
        Map<Identifier, GroupDef> defs = new HashMap<>();
    ...
    public GroupDef resolve(String group) {
        ...
        Identifier groupId = group.indexOf(':') >= 0
            ? Identifier.tryParse(group)
            : Identifier.fromNamespaceAndPath(NeoOrigins.MOD_ID, group);
        if (groupId == null) {
            warnUnknown(group);
            return GroupDef.EMPTY;
        }
