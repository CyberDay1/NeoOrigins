...
public class MorphDataManager
     ...
     */
    private static final Map<ResourceLocation, MorphSpec> BUILTINS = Map.of(
    ...
    private static MorphSpec mob(String entityType, CompoundTag nbt) {
        return new MorphSpec(
            Optional.ofNullable(ResourceLocation.tryParse(entityType)),
    ...
    private Map<ResourceLocation, MorphSpec> loaded = Map.of();
    ...
    protected Map<ResourceLocation, JsonElement> prepare(ResourceManager resourceManager, ProfilerFiller profiler) {
        ...
        for (var entry : FILE_CONVERTER.listMatchingResources(resourceManager).entrySet()) {
            ...
            ResourceLocation morphId = FILE_CONVERTER.fileToId(fileId);
    ...
    protected void apply(Map<ResourceLocation, JsonElement> object, ResourceManager resourceManager, ProfilerFiller profiler) {
        Map<ResourceLocation, MorphSpec> defs = new HashMap<>();
        for (Map.Entry<ResourceLocation, JsonElement> entry : object.entrySet()) {
            ResourceLocation morphId = entry.getKey();
            try {
    ...
    public MorphSpec resolve(String morph) {
        ...
        ResourceLocation morphId = morph.indexOf(':') >= 0
            ? ResourceLocation.tryParse(morph)
            : ResourceLocation.fromNamespaceAndPath(NeoOrigins.MOD_ID, morph);
        if (morphId == null) {
            warnUnknown(morph);
