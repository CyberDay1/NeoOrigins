...
public class MobsTargetPlayerPower extends PowerType<MobsTargetPlayerPower.Config> {
    ...
    public static class TargetOriginPlayerGoal extends NearestAttackableTargetGoal<Player> {
        ...
        public TargetOriginPlayerGoal(Mob mob) {
            ...
            //
            // 26.2 note: NearestAttackableTargetGoal.targetConditions is final on
            // this mapping, so we can't reassign it after super() the way the
            // 1.21.1 branch does. Instead the power-holder filter is passed as the
            // constructor's targetPredicate, which the base folds into its own
            // TargetingConditions. The per-power range floor is enforced inside the
            // predicate against the concrete mob (captured here), so no reassignment
            // is needed. The base scan uses the mob's follow range for the coarse
            // sweep; the predicate then trims to each holder's configured range.
            // 26.2 selector SAM is (LivingEntity, ServerLevel); level is unused.
            super(mob, Player.class, 10, false, false, (candidate, level) -> matches(mob, candidate));
        }
