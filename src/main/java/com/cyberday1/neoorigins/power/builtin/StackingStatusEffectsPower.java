...
public class StackingStatusEffectsPower extends PowerType<StackingStatusEffectsPower.Config> {
    ...
    public record Entry(ResourceLocation effect, int amplifier, boolean ambient, boolean showParticles) {
    ...
    public void onTick(ServerPlayer player, Config config) {
        for (Entry entry : config.effects()) {
            var effectOpt = BuiltInRegistries.MOB_EFFECT.getOptional(entry.effect());
