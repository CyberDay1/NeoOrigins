...
public final class BakedMeshModel {
     ...
     * PoseStack already applied). Mirrors {@link GeoJsonModel}'s Pose overload.
     */
    ...
    public void renderTinted(PoseStack poseStack, VertexConsumer consumer,
        ...
        renderTinted(poseStack.last(), consumer, r, g, b, a, packedLight, packedOverlay);
    }

    /**
     * Render with per-vertex tint color + alpha given an already-resolved
     * {@link PoseStack.Pose} (for use inside a {@code submitCustomGeometry}
     * lambda). Mirrors {@link GeoJsonModel}'s Pose overload.
     */
    public void renderTinted(PoseStack.Pose pose, VertexConsumer consumer,
                             int r, int g, int b, int a, int packedLight, int packedOverlay) {
        for (int i = 0; i < quadCount; i++) {
            for (int v = 0; v < VERTICES_PER_QUAD; v++) {
                int off = (i * VERTICES_PER_QUAD + v) * FLOATS_PER_VERTEX;
