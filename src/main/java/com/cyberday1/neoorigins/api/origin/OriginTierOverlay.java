...
public record OriginTierOverlay(
    int tier,
    List<ResourceLocation> add,
    List<ResourceLocation> remove
) {
    public static final Codec<OriginTierOverlay> CODEC = RecordCodecBuilder.create(inst -> inst.group(
        ...
        ResourceLocation.CODEC.listOf().optionalFieldOf("add", List.of()).forGetter(OriginTierOverlay::add),
        ResourceLocation.CODEC.listOf().optionalFieldOf("remove", List.of()).forGetter(OriginTierOverlay::remove)
