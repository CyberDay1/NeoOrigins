...
public class OriginCarouselSelectionScreen extends Screen implements PickerScreen {
    ...
    private void syncSelection() {
        Identifier id = browseIds.get(browseIndex);
        presenter.select(id);
    ...
    public void extractRenderState(GuiGraphicsExtractor g, int mouseX, int mouseY, float partial) {
        ...
        if (browseIds.size() > 1) {
            ...
            g.text(font, posC, panelX + panelW / 2 - font.width(posC) / 2,
    ...
    @Override
    public boolean mouseClicked(net.minecraft.client.input.MouseButtonEvent event, boolean fromWidget) {
