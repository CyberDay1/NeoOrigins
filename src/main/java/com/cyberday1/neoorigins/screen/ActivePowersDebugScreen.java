...
public class ActivePowersDebugScreen extends Screen {
    ...
    private void recomputeContentHeight() {
        Map<Identifier, Boolean> powers = ClientActivePowers.all();
        Set<String> caps = ClientActivePowers.activeCapabilities();
        int h = PAD + 9 + 6; // capabilities header
    ...
    public void extractRenderState(GuiGraphicsExtractor g, int mouseX, int mouseY, float partial) {
        ...
        // Title bar
        g.centeredText(font, Component.translatable("screen.neoorigins.debug_powers"),
        ...
        g.text(font, Component.translatable("gui.neoorigins.debug.capabilities_header"),
            panelX + PAD, sy, 0xFFCCCCDD, false);
        sy += 9 + 6;
        Set<String> caps = ClientActivePowers.activeCapabilities();
        if (caps.isEmpty()) {
            g.text(font, Component.translatable("gui.neoorigins.debug.none"),
            ...
            for (String cap : sorted) {
                ...
                g.text(font, Component.literal(cap), panelX + PAD + 8, sy, 0xFF7AACDA, false);
                sy += LINE_H;
            }
        }
        ...
        // Powers section
        g.text(font, Component.translatable("gui.neoorigins.debug.powers_header"),
            panelX + PAD, sy, 0xFFCCCCDD, false);
        sy += 9 + 6;
        Map<Identifier, Boolean> powers = ClientActivePowers.all();
        if (powers.isEmpty()) {
            ...
            List<Map.Entry<Identifier, Boolean>> entries = new ArrayList<>(powers.entrySet());
            entries.sort((a, b) -> a.getKey().toString().compareTo(b.getKey().toString()));
            for (Map.Entry<Identifier, Boolean> entry : entries) {
                ...
                g.text(font, Component.literal(label), panelX + PAD + 8, sy, textColor, false);
                sy += LINE_H;
            }
        }
        g.disableScissor();
        ...
        g.text(font, Component.literal(footer), panelX + PAD, panelBottom + 6, 0xFF556677, false);
