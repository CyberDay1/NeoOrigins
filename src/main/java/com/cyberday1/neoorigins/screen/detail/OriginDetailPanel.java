...
public final class OriginDetailPanel {
    ...
    public void renderHeader(GuiGraphics g) {
        ...
        if (origin == null) {
            ...
            g.drawString(font, hint, x + w / 2 - font.width(hint) / 2,
    ...
    public void renderBody(GuiGraphics g) {
        ...
        for (FormattedCharSequence line : descLines) {
            g.drawString(font, line, x + DETAIL_PAD, sy, theme.descriptionColor(), false);
            sy += LINE_H;
        }
        if (origin.spawnLocation().isPresent()) {
            String spawnSummary = origin.spawnLocation().get().formatSummary();
            if (!spawnSummary.isEmpty()) {
                g.drawString(font, UIThemeUtils.themed(Component.literal(spawnSummary)),
    ...
    private int renderPowers(GuiGraphics g, UITheme theme, int sy) {
        List<String> pNames = vm.powerNames();
        if (pNames.isEmpty()) return sy;
        g.drawString(font, UIThemeUtils.themedBold(Component.translatable("gui.neoorigins.detail.powers_header")),
        ...
        for (int i = 0; i < pNames.size(); i++) {
            ...
            g.drawString(font, pNameC, x + DETAIL_PAD + 8, sy, theme.powerNameColor(), false);
            // Hotkey tag (e.g. "[R]") — same slot logic as the HUD cluster.
            if (i < pTags.size() && !pTags.get(i).isEmpty()) {
                g.drawString(font, UIThemeUtils.themed(Component.literal(pTags.get(i))),
                    x + DETAIL_PAD + 8 + font.width(pNameC) + 5, sy, theme.accentColor(), false);
            }
            sy += 11;
            if (i < wrappedPowerDescs.size() && !wrappedPowerDescs.get(i).isEmpty()) {
                for (FormattedCharSequence dLine : wrappedPowerDescs.get(i)) {
                    g.drawString(font, dLine, x + DETAIL_PAD + 8, sy, theme.powerDescriptionColor(), false);
                    sy += LINE_H;
                }
            }
            sy += POWER_GAP;
     ...
     */
    private void renderEvolution(GuiGraphics g, UITheme theme, int sy) {
        ...
        if (evoOn) {
            ...
            g.drawString(font, UIThemeUtils.themed(summary), x + DETAIL_PAD, sy, theme.accentColor(), false);
            sy += LINE_H;
        }
        g.drawString(font, UIThemeUtils.themedBold(Component.translatable("gui.neoorigins.info.evolution_path")),
            x + DETAIL_PAD, sy, theme.headerColor(), false);
        sy += 9 + 4;
        for (EvoTier tier : evoTiers) {
            g.drawString(font, UIThemeUtils.themed(Component.literal(tierName(tier.tier()))),
            ...
            if (evoOn) {
                ...
                g.drawString(font, UIThemeUtils.themed(annotation),
                    x + DETAIL_PAD + 8, sy, theme.mutedColor(), false);
                sy += LINE_H;
            }
            for (EvoLine line : tier.added()) {
                g.drawString(font, UIThemeUtils.themed(Component.literal("+ " + line.name())),
                    x + DETAIL_PAD + 8, sy, theme.powerNameColor(), false);
                sy += LINE_H;
                for (FormattedCharSequence dl : line.desc()) {
                    g.drawString(font, dl, x + DETAIL_PAD + 16, sy, theme.powerDescriptionColor(), false);
                    sy += LINE_H;
                }
            }
            for (String rname : tier.removed()) {
                g.drawString(font, UIThemeUtils.themed(Component.literal("- " + rname)),
                    x + DETAIL_PAD + 8, sy, theme.mutedColor(), false);
                sy += LINE_H;
            }
        }
    ...
    private void drawImpactRow(GuiGraphics g, int cx, int rowY, Impact impact) {
        UITheme theme = UITheme.current();
        ...
        g.drawString(font, UIThemeUtils.themed(label), cx + totalW / 2 + 6, rowY - 1, theme.mutedColor(), false);
    }
     ...
     */
    public static int drawImpactDots(GuiGraphics g, int cx, int rowY, Impact impact) {
        UITheme theme = UITheme.current();
