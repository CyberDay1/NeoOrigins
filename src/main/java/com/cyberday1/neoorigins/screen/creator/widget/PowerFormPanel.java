    ...
    public void render(GuiGraphics g, int mouseX, int mouseY) {
        ...
        for (FieldRow row : rows) {
            ...
            if (scroll.rowVisible(rowTop, rh)) {
                ...
                // Hover detection uses the row's HEADER (top DEFAULT_ROW_H) so
                // RefRow's tooltip fires from its picker line, not the sub-form
                // beneath (sub-rows have their own tooltips via their hovers).
